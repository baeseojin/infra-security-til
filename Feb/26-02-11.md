...
1. systemd
    ...
    - 시스템 유닛의 활성화 상태 확인
        ...
        - `/usr/lib/systemd/system` - 유닛 원본이 들어있는 디렉터리
        - `/etc/systemd/system` - 활성화
    - 상태(state)
        - enabled - 부팅시에 실행됨
        - disabled - 부팅시에 실행 안됨
    ...
    - 유닛 제어 서브 커맨드
        - `systemctl [sub-command] [unit명]`
        - status : 상태확인
        - start : 시작
        - stop : 종료
        - restart : 재시작
        - reload : 재설정
        - enable : 활성화, `/etc/systmed/system` 디렉터리에 파일을 추가함
        - disable : 비활성화, `/etc/systemd/system`  디렉터리에 파일을 삭제
        - mask : 마스크 설정, 해당 유닛을 `/dev/null` 로 심볼릭 링크 파일을 만들어서 start를 하지 못하게 함
        - unmansk : 마스크 해제
        - list-dependencies : 종속성(의존성) 확인
    - 특정 서비스를 설치하고 앞으로도 계속해서 해당 서비스를 실행 설정할 때
        - `systemctl start xxxx.service`, `systemctl enable xxxx.service`
        - 또는 `systemctl enable xxxx.service --now`
    - systemctl 실습
    ...
    autofs 서비스 시작 후에 pid 확인
    
    systemctl start autofs
    systemctl status autofs
    pid 확인
    
    autofs 서비스 리로드 후 pid 확인
    
    systemctl reload autofs
    systemctl status autofs
    pid 확인 변화 없음
    
    autofs 서비스 재시작 후 pid 확인
...
2. log
    ...
    - 이슈 발생 시에 해당 시간에 일어났었던 일에 파악하기 위한 첫 번째 수단이기 때문에 중요하다.
    - 로그가 저장되는 위치 : /var/log
    - 로그를 관장한 서비스
        - systemd-journald
            - 로그를 수집하는 역할 (/run/)
            ...
            - `/run/log/journal` → 영구저장
                - 영구 저장 시 저장 용량 설정
                - 전체 파일 시스템 10%을 넘어서면 안됨.
                - 남아 있는 공간의 15%을 넘어서면 안됨.
                - **챗지피티는 /run/log/journal는 임시 저장이고, /var/log/journal가 영구 저장이라 함(run은 휘발성)**
                - 실습 코드
        ...
        - rsyslogd
            ...
            - `/etc/rsyslog.conf` 에서 RULES 영역에서 로그 저장 위치 설정 가능 or `/etc/rsyslog.d` 디렉터리 안에 파일 생성해서 규칙 추가하는 것도 가능
                - 해당 prioriry만 저장하고 싶으면 facility.=priority로 설정
            - rsyslog.conf 설정 변경 후 반영
                - `systemctl restart rsyslog.service` → 재시작
                - `systemctl status rsyslog` → 적용 확인
3. facility(기능)
    - authpriv : 인증
    - auth,security : login
...
5. **실시간 로그 모니터링**
    - `tail -f /var/log/로그파일`
6. 로그 메세지 발생
    - `logger -p [facility.priority]  ["메세지]"`
7. log 실습
    
    ```powershell
    rsyslog.conf 파일을 수정해서 cron.err 이상 내용만 /var/log/cron1 파일에 저장되도록 설정하시오.
    
    vi or vim /etc/rsyslog.conf
    
    /cron
    o(입력모드 키)
    cron.err         /var/log/cron1  (파일 맨 아래줄에 추가)(err(3)이상만 기록함)
    
    :wq
    
    logger 명령어를 통해서 cron.info => cronlog1 와 cron.crit => cronlog2 의 메세지를 발생시켜서 
    ...
    : cronlog2  -> crit(2)이므로 기록, info(6)은 기록 안됨
    
    journal 내용을 영구 저장하도록 설정하시오.
    
    mv /run/log/journal /var/log/
    
    ***챗지피티는 `mkdir -p /var/log/journal` 이게 맞다함***
...
📥 [리눅스 어드민 교육 3.txt](https://raw.githubusercontent.com/baeseojin/infra-security-til/main/files/리눅스%20어드민%20교육%203.txt)

📥 [리눅스 어드민 교육 3(answer).txt](https://raw.githubusercontent.com/baeseojin/infra-security-til/main/files/리눅스%20어드민%20교육%203(answer).txt)

📥 [리눅스 기초 펀드멘탈 실습3(answer).txt](https://raw.githubusercontent.com/baeseojin/infra-security-til/main/files/리눅스%20기초%20펀드멘탈%20실습3(answer).txt)
...
1. /usr/bin 디렉토리의 리스트를 출력 (상대경로)

ls ./bin/

2. /usr/lib 디렉토리의 리스트를 자세하게(-l) 출력 (절대경로)

ls -l /usr/lib/

3. /root/ 디렉토리의 리스트를 숨김파일(-a)까지 자세하게(-l) 출력 (틸드문자 사용)

ls -la ~/

4. / 디렉토리 자체(-d)의 정보를 자세하게(-l) i-node table 값(-i)까지 출력(경로:상대경로)

ls -lid ../

5. /etc/systemd/system/ 디렉토리의 리스트를 파일형식(-F)까지 나오도록 출력

ls -F /etc/systemd/system/

6. /home/vagrant 디렉토리의 하위 디렉토리 내용(-R)까지 볼수있도록 출력
...
file /etc/sysconfig => 디렉토리

cp -r /etc/sysconfig /root/dirtest - 바로 진행하면 안됨
=> dirtest 디렉토리가 없는 경우 sysconfig 디렉토리가 dirtest 디렉토리로 이름변경되서 복사됨. 그렇기 때문에 사전에 dirtest 디렉토리를 생성해줘야 함
...
file2 => file1 이름 변경  mv file2  flie1
...
touch /tmp/f1 /tmp/f2 /tmp/f3 /tmp/f4
...
5. /tmp/p 의 모든 파일의 소유자와 소유그룹을 vagrant로 변경할 것

chown -R vagrant:vagrant /tmp/p

vagrant로 사용자 전환 (su - vagrant)
...
&> or > 파일명 2>&1 : 출력결과와 에러결과를 덮어쓰기로 저장
&>> or >> 파일명 2>&1 : 출력결과와 에러결과를 이어쓰기로 저장
...
4. yum list 의 결과값 를 하위 5줄만 출력하시오.
...
+ : 앞에 있는 문자를 1번 이상 반복
. : 모든 한문자 ( ? 와 같은 의미)
.* : zero or more ( * 과 같은 의미)
[] : 대괄호 안에 문자를 하나씩 대입
[^] : 대괄호 안에 문자 제외한 한문자 (메타문자 [!] 와 같은 의미)
...
9. /root/messages 파일 내에서 12월 15일  08:00 ~ 21:59 사이의 내용을 검색하시오

grep '^Dec 15 [012][0-9]' /root/messages

원하지 않는 시간대 검색

00 01 02 03 04 05 06 07 22 23 24 25 26 27 28 29

egrep '^Dec 15 (0[89]|1[0-9]|2[01])' /root/messages

grep -e '^Dec 15 0[89]' -e '^Dec 15 1[0-9]' -e '^Dec 15 2[01]' /root/messages
...
해당 내용으로 data 파일을 생성하시오.

1. 전화번호가 중간 자리가 4자리인 내용을 출력하고 몇번째 라인에 있는지 출력하시오.

grep -n '\-....\-' data

2. seoul 이라는 내용이 있는 라인이 몇개가 있는지 출력하시오.

grep -c 'seoul' data

3. h로 시작해서 u로 끝나는 패턴을 검색하시오.

grep 'h.*u' data

4. s나 l로 시작하는 라인을 검색하시오.
...
5. 라인 뒤에 맞침표가 있는 패턴을 검색하시오.

grep '\.$' data

6. 전화번호 앞자리가 010~015까지인 패턴을 검색하시오.

grep ' 01[0-5]' data

7. 지역이 china이거나 canada인 것을 검색하시오.

egrep 'china|canada' data

혹시라도 china 가 이름인 경우

grep -v '^china' data | egrep 'china|canada'

8. n과 2라는 패턴이 동시에 들어가 있는 것을 검색하시오.
...
9. n이나 2라는 패턴이 들어가 있지 않는 것을 검색하시오.
...
4.find 명령어를 통해서 / 내에서 소유자가 vagrant 인 파일들을 모두 찾아서 자세하게 출력하시오.

5.find 명령어를 통해서 /tmp 내에서 소유자가 vagrant인 파일을 찾아서 삭제 하시오.
...
vagrant 사용자 가 매년 크리스마스 자정에 Merry christmas 메세지를 /tmp 디렉토리의 christmas 파일에 덮어쓰기로 되도록
